namespace Fluxera.Queries.Options
{
	...
	public sealed class DataQueriesOptions
	{
		...
		internal void ComplexType<T>(string complexTypeName, Action<IComplexTypeOptionsBuilder<T>> configure = null)
			where T : class
		{
		...
		/// <typeparam name="T"></typeparam>
		/// <param name="configure"></param>
		internal ComplexTypeOptions ComplexType<T>(Action<IComplexTypeOptionsBuilder<T>> configure = null)
			where T : class
		{
			if(typeof(T).IsEnumerable() || typeof(T).IsStronglyTypedId() || typeof(T).IsPrimitiveValueObject())
			{
		...
		/// <param name="configure"></param>
		/// <returns></returns>
		internal EntitySetOptions EntitySet<T>(string name, string entityTypeName, Action<IEntityTypeOptionsBuilder<T>> configure = null)
			where T : class
		{
		...
		internal EntitySetOptions EntitySet<T>(string name, Action<IEntityTypeOptionsBuilder<T>> configure = null)
			where T : class
		{
		...
		{
			return this.GetByType(typeof(T));
		}
		...
		{
			return this.GetOptionsByType(typeof(T));
		}
